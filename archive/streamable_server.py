#!/usr/bin/env python3
"""
Proxmox MCP Server - Streamable HTTP Transport
A standards-compliant MCP server using the newer Streamable HTTP transport for better compatibility
"""
...
async def run_streamable_http_server():
    ...
    logger.info(f"🚀 Starting Proxmox MCP Server (Streamable HTTP) on {host}:{port}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["--help", "-h"]:
        print("Proxmox MCP Server - Streamable HTTP Transport")
        print("Usage: python streamable_server.py")
        print()
